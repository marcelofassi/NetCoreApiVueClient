...
<script>
export default {
  name: "HojaDeTiemposCreateOrUpdate",
  data() {
    return {
      loading: false,
      form: {
  ...
  computed: {
    pageTitle() {
      return this.form.HojaDeTiemposId === 0 ? "Nuevo estudiante" : `${this.form.lastName}, ${this.form.name}`;
    }
  },
  created() {
    let self = this;
  ...
  methods: {
    get(id) {
      ...
      self.$store.state.services.HojaDeTiemposService
        ...
        .then(r => {
          self.loading = false;
          self.form.HojaDeTiemposId = r.data.HojaDeTiemposId;
    ...
    save() {
      ...
      self.$refs["ruleForm"].validate(valid => {
        if (valid) {
          ...
          if(self.form.HojaDeTiemposId > 0) {
            self.$store.state.services.HojaDeTiemposService
            .update(self.form)
            .then(r => {
              self.loading = false;
              self.$router.push('/HojaDeTiemposs');
            })
            .catch(r => {
              self.$message({
                message: "Ocurrió un error inesperado",
